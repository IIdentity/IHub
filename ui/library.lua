--// ALL CREDITS GO TO https://v3rmillion.net/member.php?action=profile&uid=1411734 \\--
for _,v in pairs(game.CoreGui:GetChildren()) do
    if v.Name == "ISU_LIB" then
        v:Destroy()
    end
end

local ISU_LIB = Instance.new("ScreenGui")
ISU_LIB.Name = "ISU_LIB"
ISU_LIB.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
ISU_LIB.Parent = game.CoreGui
...
function Library:Create(table)
    ...
    function tabHandler:Tab(name)
        ...
        main1.MouseButton1Click:Connect(function()
            for _,v in pairs(game.CoreGui:FindFirstChild('ISU_LIB').main:GetChildren()) do
                if v.Name == "container" then
                    v.Visible = false
                end
            end
            for _,v in pairs(game.CoreGui['ISU_LIB'].main.tabContainer:GetChildren()) do
        ...
        function ElementHandler:Toggle(text, callback)
            ...
            toggle1.MouseButton1Click:Connect(function()
                ...
                callback(tog)
                if tog then
                    game:GetService('TweenService'):Create(frame3, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(31, 154, 236)}):Play()
                    game:GetService('TweenService'):Create(uIStroke1, TweenInfo.new(0.2), {Color = Color3.fromRGB(23, 119, 182)}):Play()
                else
    ...
    main.MouseEnter:Connect(function()
        local Input = main.InputBegan:connect(function(Key)
            if Key.UserInputType == Enum.UserInputType.MouseButton1 then
                ...
                while RunService.RenderStepped:wait() and UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) or UserInputService:IsMouseButtonPressed(Enum.UserInputType.Touch) do
                    local FrameX, FrameY = math.clamp(Mouse.X - ObjectPosition.X, 0, ISU_LIB.AbsoluteSize.X - main.AbsoluteSize.X), math.clamp(Mouse.Y - ObjectPosition.Y, 0, ISU_LIB.AbsoluteSize.Y - main.AbsoluteSize.Y)
    ...
    game.CoreGui['ISU_LIB'].main.tabContainer.ChildAdded:Connect(function()
        game.CoreGui['ISU_LIB'].main:FindFirstChild('container').Visible = true
        pcall(function()
            repeat wait() until game.CoreGui['ISU_LIB'].main:FindFirstChild('tabContainer'):FindFirstChildWhichIsA('TextButton')
        end)
        game:GetService('TweenService'):Create(game.CoreGui['ISU_LIB'].main:FindFirstChild('tabContainer'):FindFirstChildWhichIsA('TextButton'), TweenInfo.new(0.3), {TextTransparency = 0}):Play()
    end)
